...
If sync is deployed into openshift, an RBAC rule must be provided to the `greymatter-sync` service account
to allow anyuid:

```bash
oc create -f openshift/manifests.yaml -n %MY_NAMESPACE # this file contains RBAC bindings
```

Once you have created the role and its binding, you must add a security context to the sync manifest:
```yaml
  securityContext:
    runAsUser: 1000
